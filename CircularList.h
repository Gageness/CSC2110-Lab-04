...
DoubleNode<T>* CircularList<T>::find(int index) 
{
   ...
   {
        dist_next = (sze - loc_pos) + index;      //distance using the bridge (prev refs, negative)
   ...
   else if (abs(dist_next) > abs(dist_prev)) {
...
void CircularList<T>::remove(int index) 
{
   ...
   if (index >= 1 && index <= sze) 
   {
      ...
      {
         DoubleNode<T>* curr = index;
		 DoubleNode<T>* prev = find(index - 1);
		 DoubleNode<T>* next = getNext(curr);

		 prev = getNext(curr -> next);
